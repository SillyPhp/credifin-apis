...
class EmiCollectionsController extends ApiBaseController
{
    ...
    public function actionCollectionDailyStats()
    {
        ...
        foreach ($valuesSma as $key => $value) {
            $inClause = "('" . implode("', '", $value['subBucket']) . "')";
            $select[] = "COALESCE(COUNT(CASE WHEN lac.sub_bucket IN $inClause AND ec.amount > 0 THEN ec.amount END), 0) {$key}_total_cases_count";
            ...
            $select[] = "COALESCE(COUNT(CASE WHEN lac.sub_bucket IN $inClause AND lap.id IS NOT NULL AND ec.amount > 0 AND ec.created_on BETWEEN '{$startDate}' AND '{$endDate}' THEN ec.amount END), 0) {$key}_total_interaction_count";
