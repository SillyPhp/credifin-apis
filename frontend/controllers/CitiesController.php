...
class CitiesController extends Controller
{
    ...
    private function getInstitutes($city, $type = null)
    {
        ...
        $organizations = Organizations::find()
            ->alias('a')
            ->select(['distinct(a.organization_enc_id)', 'a.name', 'a.initials_color', 'a.slug', 'CASE WHEN a.logo IS NOT NULL THEN CONCAT("' . Url::to(Yii::$app->params->digitalOcean->organizations->logo) . '", a.logo_location, "/", a.logo) ELSE NULL END image', '(CASE WHEN a.organization_enc_id IS NOT NULL THEN "claimed" END) as org_type'])
            ->joinWith(['organizationLocations b' => function ($b) {
                $b->joinWith(['cityEnc c']);
            }], false)
        ...
        $un_org = UnclaimedOrganizations::find()
            ->alias('a')
            ->select(['distinct(a.organization_enc_id)', 'a.name', 'a.initials_color', 'a.slug', 'CASE WHEN a.logo IS NOT NULL THEN CONCAT("' . Url::to(Yii::$app->params->digitalOcean->organizations->logo) . '", a.logo_location, "/", a.logo) ELSE NULL END image', '(CASE WHEN a.organization_enc_id IS NOT NULL THEN "unclaimed" END) as org_type'])
