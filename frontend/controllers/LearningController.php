...
class LearningController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $object = QuestionsPool::find()
            ->alias('a')
            ->andWhere(['a.is_deleted'=>0])
            ->select(['a.question_pool_enc_id','c.name','question','privacy','a.slug','CASE WHEN f.image IS NOT NULL THEN CONCAT("' . Url::to(Yii::$app->params->digitalOcean->users->image) . '", f.image_location, "/", f.image) ELSE NULL END image','f.username','f.initials_color','CONCAT(f.first_name," ","f.last_name") user_name'])
            ...
            ->joinWith(['questionsPoolAnswers d'=>function($b)
            {
                ...
                $b->select(['d.question_pool_enc_id','CONCAT(e.first_name," ",e.last_name) name','CASE WHEN e.image IS NOT NULL THEN CONCAT("' . Url::to(Yii::$app->params->digitalOcean->users->image) . '", e.image_location, "/", e.image) ELSE NULL END image','e.username','e.initials_color']);
                $b->limit(3);
            }])
            ->limit(6)
            ->asArray()
    ...
    public function actionGetParentComments()
    {
        if (Yii::$app->request->isAjax && Yii::$app->request->isPost) {
            ...
            $result = LearningVideoComments::find()
                ->alias('a')
                ->select(['a.comment_enc_id', 'a.comment reply', 'b.username', 'CONCAT(b.first_name, " ", b.last_name) name', 'b.initials_color color', 'CASE WHEN b.image IS NOT NULL THEN CONCAT("' . Url::to(Yii::$app->params->digitalOcean->users->image) . '", b.image_location, "/", b.image) ELSE NULL END img'])
        ...
        if (Yii::$app->request->isAjax && Yii::$app->request->isPost) {
            ...
            if ($a = $this->saveComment($comment, $learning_video['video_enc_id'], $current_user, NULL)) {
                $user_info = [
                    ...
                    'path' => Yii::$app->params->digitalOcean->users->image . Yii::$app->user->identity->image_location . DIRECTORY_SEPARATOR . Yii::$app->user->identity->image,
    ...
    private function __getContributors($limit = null)
    {
        $contributors = Users::find()
            ...
            ->select([
                ...
                'CASE WHEN a.image IS NOT NULL THEN CONCAT("' . Url::to(Yii::$app->params->digitalOcean->users->image, 'https') . '", a.image_location, "/", a.image) ELSE "/assets/themes/ey/images/pages/learning-corner/collaborator.png" END image'
            ])
