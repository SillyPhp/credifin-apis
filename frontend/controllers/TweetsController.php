...
class TweetsController extends Controller
{
    ...
    public function actionFetchTweetsCards()
    {
        if (Yii::$app->request->isPost) {
            ...
            $tweets = TwitterJobs::find()
                ->alias('a')
                ->distinct()
                ->select(['a.job_type', 'c.name org_name', 'a.html_code', 'f.name profile', 'e.name job_title', 'c.initials_color color', 'CASE WHEN c.logo IS NOT NULL THEN  CONCAT("' . Url::to(Yii::$app->params->digitalOcean->unclaimedOrganizations->logo) . '",c.logo_location, "/", c.logo) END logo'])
    ...
    private function _getTweets($keywords = null, $location = null, $type = null, $limit = null, $offset = null)
    {
        $tweets1 = (new \yii\db\Query())
            ->distinct()
            ->select(['a.tweet_enc_id', 'a.job_type', 'a.created_on', 'c.name org_name', 'a.html_code', 'f.name profile', 'e.name job_title', 'c.initials_color color', 'CASE WHEN c.logo IS NOT NULL THEN  CONCAT("' . Url::to(Yii::$app->params->digitalOcean->unclaimedOrganizations->logo) . '",c.logo_location, "/", c.logo) END logo'])
        ...
        $tweets2 = (new \yii\db\Query())
            ->distinct()
            ->select(['a.tweet_enc_id', 'a.job_type', 'a.created_on', 'c.name org_name', 'a.html_code', 'f.name profile', 'e.name job_title', 'c.initials_color color', 'CASE WHEN c.logo IS NOT NULL THEN  CONCAT("' . Url::to(Yii::$app->params->digitalOcean->organizations->logo) . '",c.logo_location, "/", c.logo) END logo'])
            ->from(TwitterJobs::tableName() . 'as a')
            ->leftJoin(TwitterPlacementCities::tableName() . ' g', 'g.tweet_enc_id = a.tweet_enc_id')
            ->leftJoin(Cities::tableName() . 'as h', 'h.city_enc_id = g.city_enc_id')
            ->innerJoin(AssignedCategories::tableName() . 'as d', 'd.assigned_category_enc_id = a.job_title')
