        ...
        if (Yii::$app->request->isAjax) {
            if (!Yii::$app->user->isGuest) {
                ...
                $review_list = ReviewedApplications::find()
                    ->alias('a')
                    ->select(['a.review_enc_id', 'a.application_enc_id as application_id', 'CONCAT(a.application_enc_id, "-", f.location_enc_id) data_key', 'a.review', 'd.name as title', 'b.slug', 'e.initials_color color', '(CASE WHEN e.name IS NOT NULL THEN e.name ELSE i.name END) as org_name', 'CASE WHEN e.logo IS NOT NULL THEN CONCAT("' . Url::to(Yii::$app->params->digitalOcean->organizations->logo) . '", e.logo_location, "/", e.logo) ELSE NULL END logo'])
