...
$data = [
    'title' => $this->title,
    'background' => Url::to('@eyAssets/images/backgrounds/bg1.png'),
];
...
<section>
    <div class="container mt-30 mb-5 pt-30 pb-5">
        <div class="row">
            <div class="col-md-6">
                        ...
                        <div class="post-thumb">
                            <div class="owl-carousel-1col">
                                ...
                                foreach ($posts as $post) {
                                    ...
                                    ?>
                                    <div class="item">
                                        <a href="<?= Url::to('/blog/' . $post['slug']); ?>"><img src="<?= Url::to($image); ?>" width="570" height="325" alt="<?= $post['featured_image_alt']; ?>" title="<?= $post['featured_image_title']; ?>"/> </a>
                                        <div class="carousel-content"  style=" background-color:#f5d4bc;">
                                        </div>
                                        <div class="tag" style="text-align:justify;padding-left:5px;padding-right:10px;font-family:crimson-text;font-size:12pt;">
                                            <a href="<?= Url::to('/blog/' . $post['slug']); ?>" style="color:black; font-weight:bolder;padding-left:5px;text-align:justify;text-transform: uppercase;font-size:20px;"><?= $post['title']; ?></a><br>
                                            <?php
                                            if (strlen($post['excerpt']) > 55) {
                                                echo substr($post['excerpt'], 0, 55) . ' ....<a href="' . Url::to('/blog/' . $post['slug']) . '">Read More</a>';
                                            } else {
                                                echo $post['excerpt'] . ' ... ';
                                                ?>
                                                <a href="<?= Url::to('/blog/' . $post['slug']); ?>" style="color:red;padding-left:5px;font-size:16px;">Read More</a>
                                                <?php
                                            }
                                            ?>
                                        </div>
            ...
            foreach ($similar_posts as $post) {
                ...
                $image = Yii::$app->params->upload_directories->posts->featured_image . $post['featured_image_location'] . DIRECTORY_SEPARATOR . $post['featured_image'];
                if (!file_exists($image_path)) {
                    $image = '//placehold.it/285x190';
                }
                ?>          
                <div class="col-md-3 col-sm-6">
                    <article class="post clearfix mb-5 pb-5">
                        <div class="entry-header">
                            <div class="item">
                                <a href="<?= Url::to('/blog/' . $post['slug']); ?>"><img src="<?= Url::to($image); ?>" width="570" height="133" alt="<?= $post['featured_image_alt']; ?>" title="<?= $post['featured_image_title']; ?>"/> </a>
                            </div>
                            <div class="carousel-content" style=" background-color:#f5d4bc;">
                            </div>
                            <div class="caption" style="text-align:left;padding-left:5px;padding-right:10px;">
                                <a href="<?= Url::to('/blog/' . $post['slug']); ?>" style="color:black; font-weight: bold;padding-left:5px;text-align:left;font-family:crimson-text;font-size: 12pt;"><?= $post['title']; ?></a>
                                <?php
                                if (strlen($post['excerpt']) > 5) {
                                    $stringCut = substr($post['excerpt'], 0, 5);
                                    $post['excerpt'] = substr($post['excerpt'], 0, 5) . ' ... <a href="' . Url::to('/blog/' . $post['slug']) . '">Read More</a>';
                                }
                                echo $post['excerpt'];
                                ?>
                            </div>
...
<div id="loading" class="text-center mb-20">
    <img src="<?= Url::to('@eyAssets/images/preloaders/8.gif'); ?>" alt="<?= Yii::t('frontend', 'Loading'); ?>…" />
</div> 
                                    ...
                                    foreach ($quotes as $post) {
                                        ...
                                        <div class="zoom">
                                            ...
                                            <div class="carousel-content">
                                                <a href="<?= Url::to('/blog/' . $post['slug']); ?>"><?= $post['']; ?></a>
                                            </div>
                                        </div>
                                        <?php
                                    }
