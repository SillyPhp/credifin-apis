...
class Applications extends EmployerApplications
{
    ...
    public function getApplications($options = [])
    {
        ...
        $applications = self::find()
            ->alias('a')
            ->distinct()
            ->select(['a.application_enc_id','interview_process_enc_id','g.positions','a.type','a.last_date','a.title', 'CONCAT("' . $slug . '", a.slug) link', 'c.name as name', 'd.icon', 'LOWER(f.name) application_type'])
            ...
            if($options['options']['placement_locations']) {
                $applications->joinWith(['locations']);
            }
            $applications->where(['a.is_deleted' => 0])
