...
class UserAppliedApplication
{
    public function getUserDetails($type,$limit=null)
    {
        $u = AppliedApplications::find()
             ->alias('a')
            ->select(['a.status','a.created_on','g.name type','a.applied_application_enc_id', 'a.application_enc_id', 'a.rejection_window', 'f.username', 'd.name job_title', 'e.icon', 'CONCAT(f.first_name," ",f.last_name) fullname', 'f.image', 'f.image_location'])
             ->where(['or',
                 ['a.status' => 'Pending'],
                 ['a.status' => 'Incomplete']
             ])
...
//            ->limit(10)
        ...
        foreach ($db as $key=>$value)
        {
            $a =  AppliedApplications::find()->alias('aa')
                ->select(['aa.applied_application_enc_id','aa.application_enc_id', 'aa.created_on', 'aa.status', 'aa.rejection_window','COUNT(CASE WHEN c.is_completed = 1 THEN 1 END) as active','aa.created_by', 'aa.resume_enc_id','e.resume', 'e.resume_location','b.user_enc_id','b.username', 'CONCAT(b.first_name, " ", b.last_name) name','CASE WHEN b.image IS NOT NULL THEN CONCAT("' . Url::to(Yii::$app->params->upload_directories->users->image) . '", b.image_location, "/", b.image) ELSE NULL END image',])
