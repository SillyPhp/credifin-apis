...
//                        'class' => "form-inline",
            ...
            ?>
            <?= Html::submitButton('Save', ['class' => 'btn blue btn-circle submit pull-right']); ?>
            <?= $form->field($model, 'title')->textInput(['id' => 'title','placeholder' => 'Title','value' => $process['process_name']])->label(false); ?>
            ...
            <div class="col-md-4">
                <div class="draggable-main">
                    <ul class="connected-sortable draggable-left">
                        ...
                        <li class="c"><i class="fa fa-cogs" aria-hidden="true"></i> Technical Inteview</li>
                        ...
                        <!-- <li class="i"><i class="fa fa-arrows" aria-hidden="true"></i> Questionnaire</li> -->
            ...
            <div class="col-md-8">
                <div class="box">
                    ...
                    <ul class="connected-sortable draggable-right">
                        <?php
                        if(!empty($process)) {
                            $total_process = count($process['interviewProcessFields'])-1;
                            for($i=1; $i<$total_process; $i++) {
                                ?>
                                <div class="li_<?= $process['interviewProcessFields'][$i]['field_enc_id'] ?> form_builder_field" style="width: 866px; height: 63px;"><li class="form_output" data-type="interview_process"><i class="<?= $process['interviewProcessFields'][$i]['icon'] ?>" aria-hidden="true"></i><?= $process['interviewProcessFields'][$i]['field_name'] ?><a href="#" class="edit_process" data-field="<?= $process['interviewProcessFields'][$i]['field_enc_id'] ?>"><i class="fa fa-pencil-square-o"></i></a><a href="#" class="remove_process" data-field="<?= $process['interviewProcessFields'][$i]['field_enc_id'] ?>"><i class="fa fa-times"></i></a><ul class="process_desc"><textarea type="text" name="name_30771" placeholder="Add Description(optional)" class="form-control custom_font"></textarea></ul></li></div>
                                <?php
                            }
                        }
                        ?>
...
$(".draggable-right").sortable({
    ...
    $('#process_form').validate({
       ...
       rules:   {
                    'InterviewProcess[process_validate]':
                     {
                       required:true,
                     },
        },
