                    ...
                    <div class="wizard card-like">
                        <form id="add-applications-inErexx">
                            <div class="wizard-header">
                                <div class="row">
                                    <div class="col-xs-12">
                                        <a href="<?= Url::to('/account/'. $type .'/dashboard')?>" id="wizard-back-cont" type="button" class="wizard-back-cont">
                                    ...
                                    <div class="row">
                                        ...
                                        if (!empty($applications['data'])) {
                                            ...
                                            <div class="col-md-12 text-center">
                                                <h4 class="slct">Select An Application For Campus Hiring.</h4>
                                            </div>
                                            <?php
                                            foreach ($applications['data'] as $app) {
                                                ?>
                                                <div class="col-sm-6 app-list-data-main">
                                                    <input id="<?= $app['application_enc_id']; ?>" type="radio"
                                                           name="applications"
                                            ...
                                            ?>
                                            <div class="col-md-12 text-center">
                                                <h2>No Active <?= $ucType ?> found!</h2>
                                                <h4>Create AI <?= $ucType ?> to Hire Candidates from Colleges</h4>
                                                <a href="<?= Url::to('/account/'.$type.'/create');?>" class="btn btn-primary">Create AI <?= $ucType ?></a>
                                            </div>
                                            <?php
                                        }
                                        ?>
    ...
    if(valdidate_form){
        sessionStart();
         ...
         $('.wizard-prev').prop('disabled', true);
                   swal({
                        title: "",
                        text: "Do You Want To Make Changes Or Clone as It Is?",
                        type:'warning',
                        showCancelButton: true,  
                        confirmButtonClass: "btn-primary",
                        confirmButtonText: "No, Just Clone Without Changes",
                        cancelButtonText: "Edit And Clone Application",
                        closeOnConfirm: true, 
                        closeOnCancel: true
                         },
                            function (isConfirm) { 
                             if (isConfirm){           
                                 ...
                                 window.location.href = '/account/$type/'+$("input[name='applications']:checked").val()+'/clone';
                             }
                         }
                        );
    } else {
         swal({
               title:"Wait !!!",
               text: "Please Select Atleast One College To Proceed",
               type:'warning',
               });
...
function sessionStart(){
  ...
  $.ajax({
    url: '/account/jobs/store-session',
    type: 'post',
    data: data,
    success:function(res) {
      if (res.status==200){
          ...
          {
              console.log('failed to stored session');
          }
    }
  })
}
   ...
   var me = $('.submit-applications-inErexx');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // function explode(){
                     window.location.replace('/account/$type/dashboard'); 
                // }
